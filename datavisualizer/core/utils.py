...
def two_bar_chart(x, y, z):
    ...
    plt.bar(x_axis + 0.21, z, 0.4, label="Asistencije", color='#5DBE7C', edgecolor='#20884B', linewidth=2, zorder=3)
...
def horizontal_barchart(x, y, z):
    ...
    plt.barh(y_axis + 0.22, z, 0.4, label='Asistencije', color='#5DBE7C', edgecolor='#20884B', linewidth=2, zorder=3)
...
def performance_check(x, y, z, name):
    ...
    plt.plot(x, y, label=name, color='#008b8b', linewidth=2, marker='o', markersize=5, markerfacecolor='cyan', zorder=3)
    plt.plot(x, z, label="Prosjek", color='#95F7B1', linewidth=2, marker='o', markersize=5, markerfacecolor='#20884B', zorder=3)
    legend = plt.legend()
    legend_text = legend.get_texts()
    plt.setp(legend_text, fontname='Calibri', fontsize=14, fontweight='bold')
    plt.tight_layout()
